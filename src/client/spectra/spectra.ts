...
import CMF_5 from "./cie1931-xyzbar-380+5+780";
import illuminant_d65_5 from "./illuminant-d65-380+5+780";
import illuminant_c_5 from "./illuminant-c-380+5+780";
import basis from "./cie1931-basis-bt709-380+5+780";
...
export const xyY_to_XYZ = (arg: number[]) => {
  var X, Y, Z, x, y;
  x = arg[0];
  y = arg[1];
  Y = arg[2];
...
export const xyz_to_xyY = (arg: number[]) => {
  var sum, X, Y, Z;
  X = arg[0];
  Y = arg[1];
  Z = arg[2];
...
const gamma_to_linear = (c: number) =>
  c >= 0.04045 ? Math.pow((c + 0.055) / 1.055, 2.4) : c / 12.92;

const linear_to_gamma = (c: number) =>
...
function linear_to_concentration(l1: number, l2: number, t: number) {
  let t1 = l1 * (1 - t) ** 2;
  let t2 = l2 * t ** 2;
  return t2 / (t1 + t2);
}

export function mix_spectra(R1: number[], R2: number[], t: number) {
...
export function spectra_to_XYZ(
  reflectance: number[],
...
export function XYZ_to_linearRGB(XYZ: number[]) {
  let [x, y, z] = XYZ;
...
export function wavelength_to_sRGB(len: number) {
  return XYZ_to_sRGB(wavelength_to_XYZ(len));
}

export function wavelength_to_XYZ(len: number) {
...
export function XYZ_to_sRGB(XYZ: number[]) {
  return linearRGB_to_sRGB(XYZ_to_linearRGB(XYZ));
}

export function spectra_to_sRGB(reflectance: number[]) {
